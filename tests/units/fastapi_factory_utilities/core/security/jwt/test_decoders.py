    ...
    async def test_decode_with_audience(
        ...
        with patch("fastapi_factory_utilities.core.security.jwt.decoders.decode") as mock_decode:
            ...
            mock_decode.assert_called_once_with(
                ...
                options={"verify_signature": True},
                issuer=config_with_audience.issuer,
    ...
    async def test_decode_with_all_options(
        ...
        with patch("fastapi_factory_utilities.core.security.jwt.decoders.decode") as mock_decode:
            ...
            mock_decode.assert_called_once_with(
                ...
                options={"verify_signature": True},
                issuer=config_with_all_options.issuer,
                audience=config_with_all_options.authorized_audiences,
                subject=subject,
            )

    @pytest.mark.asyncio
    async def test_decode_with_explicit_issuer_overrides_config_issuer(
        self,
        jwt_token: JWTToken,
        mock_public_key: MagicMock,
        minimal_config: JWTBearerAuthenticationConfig,
        ...
        """Test that passing explicit issuer to decode_jwt_token_payload overrides config.issuer.

        Args:
            jwt_token (JWTToken): The JWT token.
            mock_public_key (MagicMock): The mock public key.
            minimal_config (JWTBearerAuthenticationConfig): The config (has different issuer).
            decoded_payload (dict[str, Any]): The decoded payload.
        """
        explicit_issuer: OAuth2Issuer = OAuth2Issuer("https://explicit-issuer.example")
        with patch("fastapi_factory_utilities.core.security.jwt.decoders.decode") as mock_decode:
            mock_decode.return_value = decoded_payload
...
class TestGenericJWTBearerTokenDecoder:
    """Various tests for the GenericJWTBearerTokenDecoder class."""
    ...
    def mock_jwks_store(self) -> MagicMock:
        ...
        store.get_issuer_by_kid = AsyncMock(return_value=OAuth2Issuer("https://example.com"))
        return store

    @pytest.fixture
    def decoder(
        self, jwt_config: JWTBearerAuthenticationConfig, mock_jwks_store: MagicMock
    ) -> GenericJWTBearerTokenDecoder[JWTPayload]:
        ...
        Returns:
            GenericJWTBearerTokenDecoder[JWTPayload]: A JWT bearer token decoder.
        """
        decoder_instance = GenericJWTBearerTokenDecoder[JWTPayload](
    ...
    async def test_decode_payload_success(
        ...
        Args:
            decoder (GenericJWTBearerTokenDecoder[JWTPayload]): The decoder instance.
            jwt_token (JWTToken): The JWT token.
        ...
        with patch("fastapi_factory_utilities.core.security.jwt.decoders.get_unverified_header") as mock_get_header:
            ...
            with patch("fastapi_factory_utilities.core.security.jwt.decoders.decode_jwt_token_payload") as mock_decode:
                ...
                mock_jwks_store.get_issuer_by_kid.assert_called_once_with(kid="test_kid")
                mock_decode.assert_called_once_with(
                    jwt_token=jwt_token,
                    public_key=mock_jwk,
    ...
    @pytest.mark.asyncio
    async def test_decode_payload_returns_custom_payload_subtype(
        self,
        ...
        """Test that GenericJWTBearerTokenDecoder returns the generic payload model (e.g. JWTPayload subclass).
        ...
        class CustomJWTPayload(JWTPayload):
            """Minimal subclass of JWTPayload for testing generic decoder."""

            pass

        decoder = GenericJWTBearerTokenDecoder[CustomJWTPayload](
            jwt_bearer_authentication_config=jwt_config,
            jwks_store=mock_jwks_store,
        )
        ...
        with patch("fastapi_factory_utilities.core.security.jwt.decoders.get_unverified_header") as mock_get_header:
            ...
            with patch("fastapi_factory_utilities.core.security.jwt.decoders.decode_jwt_token_payload") as mock_decode:
                ...
                assert isinstance(result, CustomJWTPayload)
                assert isinstance(result, JWTPayload)
                assert result.scp == ["read", "write"]
                assert result.sub == "test_subject"
